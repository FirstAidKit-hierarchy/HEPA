import { FileBarChart, GraduationCap, HeartPulse, Search } from "lucide-react";
    ...
    <section id="capabilities" className="bg-section-gradient py-16 sm:py-20">
      <div className="section-container">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(260px,0.42fr)] lg:items-end">
          <Reveal>
            <SectionHeading align="left" eyebrow={solutions.eyebrow} title={solutions.title} description={solutions.description} />
          </Reveal>
          <Reveal delay={120}>
            <div className="rounded-[1.8rem] border border-border/70 bg-background/85 p-5 shadow-[0_18px_44px_rgba(15,23,42,0.08)]">
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-accent-blue">{solutions.sidePanel.eyebrow}</p>
              <p className="mt-4 text-sm leading-7 text-muted-foreground">{solutions.sidePanel.description}</p>
            </div>
          </Reveal>
        </div>
        <div className="mt-12 grid gap-5 sm:grid-cols-2 sm:gap-6">
          {solutions.cards.map((solution, index) => {
            const accent = accentClasses[solution.accent] ?? accentClasses.accentBlue;
            const Icon = solutionIcons[solution.iconKey as keyof typeof solutionIcons] ?? FileBarChart;
            ...
            return (
              <Reveal key={solution.title} delay={index * 90}>
                ...
                >
                  <div className={`mb-5 flex h-12 w-12 items-center justify-center rounded-xl transition-colors duration-500 ${accent.iconWrap}`}>
